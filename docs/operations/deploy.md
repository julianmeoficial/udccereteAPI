...
**Estado:** diseño (stubs en el repo) · **Actualizado:** 2026-08-25
...
| `docker/api.Dockerfile` | Stub (`node:24-alpine`, imprime la versión de Node). |
| `docker/Caddyfile` | Comentario de ejemplo; sin host real. |
| GitHub Actions | [CI placeholder](ci.md). No hay job de deploy SSH. |

## Objetivo (cuando se implemente)

1. Imagen de `@udccerete/api` escuchando en `PORT` (default 3001).
